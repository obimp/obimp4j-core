 ...
 */
class Packet(val type: Short, val subtype: Short) {
    private val header = Header(type.toBytes(), subtype.toBytes())
    private var data = mutableListOf<WTLD>()
    ...
    fun setData(data: MutableList<WTLD>) {
        this.data = data
    }

    fun toBytes(): ByteArray {
        header.length = (data.size * 8) + data.sumOf(WTLD::length)
        return header.getBytes() + data.map(WTLD::toBytes).fold(byteArrayOf(), ByteArray::plus)
    }
