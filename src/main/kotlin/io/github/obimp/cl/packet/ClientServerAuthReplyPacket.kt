 ...
 */
class ClientServerAuthReplyPacket(
    accountName: String,
    reply: AuthorizationReply,
    offlineFlag: Boolean = false,
    offlineTime: LocalDateTime? = null,
    transportItemID: Int? = null
) : OBIMPPacket(OBIMPHeader(type = OBIMP_BEX_CL, subtype = OBIMP_BEX_CL_CLI_SRV_AUTH_REPLY)) {
    init {
        addItem(WTLD(LongWord(0x0001), UTF8(accountName)))
        addItem(WTLD(LongWord(0x0002), Word(reply.code)))
        if (offlineFlag) {
